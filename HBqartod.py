...
def spike_test(inp: Sequence[N],
    ...
    Args:
        ...
        method: ['average'(default),'differential', 'median'] optional input to assign the method used to detect spikes.
            ...
            "median": Determine if there is a spike at datapoint n by calculating the difference 
                between n and the median or n-1, n and n+1 and taking the absolute of this quantity, 
                and checking if it exceeds a high or low threshold.
    Returns:
        A masked array of flag values equal in size to that of the input.
    """
    ...
    if method == 'average':
        ...
        # Set number of data points to take the median over.
        L = 3
        # Calculate number of rows in an array with length L values per row, stepsize =1, 
        nrows = (inp.size-L)+1
        # create an array of the median of the 3 values 
        med = np.median(inp[np.arange(nrows)[:,None] + np.arange(L)], axis=1)
        # duplicate the first and last value so that the array is the same length as the input
        med = np.insert(med, 0, med[0], axis=0)
        med = np.insert(med, -1, med[-1], axis=0)
        diff = np.abs(inp - med)
    else:
        raise ValueError(
